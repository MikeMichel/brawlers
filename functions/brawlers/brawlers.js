...
exports.fetchDataAndSaveToFirestore = functions.https.onRequest(async (req, res) => {
  try {
    ...
    data.forEach(async (item) => {
      const documentSnapshot = await collectionRef.where("id", "==", item.id).get(); // Check if a document with the same "id" exists
