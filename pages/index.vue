...
const { data: brawlers } = await useFetch(
...
const filteredBrawlers = computed(() => {
  ...
  const filtered = brawlers.value.list.filter((brawler) => {
    ...
    const result =
      (rarity.value === "All" || brawler.rarity.name === rarity.value) &&
...
const animatedValue = ref(filteredBrawlers.value.length); // filteredBrawlers.value.length is the initial value
watch(filteredBrawlers, () => {
  gsap.to(animatedValue, {
    duration: 1,
    value: filteredBrawlers.value.length,
    // no decimals
    roundProps: "value",
  });
});
...
<template>
  ...
  <div class="p-4 xl:p-0">
    ...
    <select
      ...
      class="w-1/2 px-4 py-2 text-base text-gray-700 placeholder-gray-600 border rounded-lg focus:shadow-outline"
    >
      <option v-for="item in rarity_items" :key="item" :value="item">
        {{ item }}
      </option>
    </select>
  </div>
    ...
    >
      <div class="card h-min w-full bg-base-100 shadow-xl">
        ...
        <div class="card-body">
          ...
          <h4 class="badge md:badge-sm badge-secondary">
            {{ brawler.rarity.name }}
          </h4>
          <h4 class="badge md:badge-sm badge-primary">
            {{ brawler.class.name }}
          </h4>
